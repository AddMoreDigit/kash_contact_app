...
async function registerUser(data) {
  ...
  try {
    ...
    if (exists.rowCount > 0) {
      await client.end();
      return { statusCode: 409, headers: CORS_HEADERS, body: JSON.stringify({ success: false, message: 'Email already registered' }) };
    }
    ...
    return { statusCode: 201, headers: CORS_HEADERS, body: JSON.stringify({ success: true, message: 'User registered. Verification code sent to email.', data: { userId: res.rows[0].id } }) };
  } catch (err) {
    console.error('registerUser error', err);
    try { await client.end(); } catch (e) {}
    return { statusCode: 500, headers: CORS_HEADERS, body: JSON.stringify({ success: false, message: err.message }) };
  }
}

async function verifyEmail(data) {
  const { email, otpCode } = data;
  if (!email || !otpCode) return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ success: false, message: 'Missing email or otpCode' }) };
  const client = await getDbClient();
  try {
    const res = await client.query('SELECT id, otp_code, otp_expires_at FROM users WHERE email = $1', [email]);
    if (res.rowCount === 0) {
      await client.end();
      return { statusCode: 404, headers: CORS_HEADERS, body: JSON.stringify({ success: false, message: 'User not found' }) };
    }
    const user = res.rows[0];
    if (!user.otp_code || user.otp_code !== otpCode) {
      await client.end();
      return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ success: false, message: 'Invalid code' }) };
    }
    if (new Date(user.otp_expires_at) < new Date()) {
      await client.end();
      return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ success: false, message: 'Code expired' }) };
    }
    ...
    await client.end();
    return { statusCode: 200, headers: CORS_HEADERS, body: JSON.stringify({ success: true, message: 'Email verified' }) };
  } catch (err) {
...
async function loginUser(data) {
  ...
  if (!email || !password) return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ success: false, message: 'Missing email or password' }) };
  const client = await getDbClient();
  try {
    const res = await client.query('SELECT id, password_hash, email_verified, user_type FROM users WHERE email = $1', [email]);
    if (res.rowCount === 0) {
      await client.end();
      return { statusCode: 401, headers: CORS_HEADERS, body: JSON.stringify({ success: false, message: 'Invalid credentials' }) };
    }
    const user = res.rows[0];
    ...
    if (!user.email_verified) {
      await client.end();
      return { statusCode: 403, headers: CORS_HEADERS, body: JSON.stringify({ success: false, message: 'Email not verified' }) };
    }
    ...
    await client.end();
    return { statusCode: 200, headers: CORS_HEADERS, body: JSON.stringify({ success: true, data: { token } }) };
  } catch (err) {
...
async function resendOtp(data) {
  const { email } = data;
  if (!email) return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ success: false, message: 'Missing email' }) };
  const client = await getDbClient();
  try {
    ...
    if (user.email_verified) {
      await client.end();
      return { statusCode: 400, headers: CORS_HEADERS, body: JSON.stringify({ success: false, message: 'Email already verified' }) };
    }
    ...
    await sendVerificationEmail(email, otp);
    await client.end();
    return { statusCode: 200, headers: CORS_HEADERS, body: JSON.stringify({ success: true, message: 'Verification code resent' }) };
  } catch (err) {
...
exports.handler = async (event) => {
  ...
  try {
    ...
    return { statusCode: 404, headers: CORS_HEADERS, body: JSON.stringify({ success: false, message: 'Not found' }) };
  } catch (err) {
